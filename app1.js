...
function selectWinnerBoxes(b1, b2, b3) {
  ...
  end.innerHTML = b1.innerHTML + " Won Congrat";
...
for (var i = 0; i < boxes.length; i++) {
  boxes[i].onclick = function() {
	    ...
	    //prevent further click if there already a winner
	    if (gotWinner) {
	      return false;
	    }
	    ...
	    if (this.innerHTML !== "X" && this.innerHTML !== "O") {
	      ...
	      if (X_or_O == boxes.length && !gotWinner) {
	      	turn.innerHTML = "No Winner. Is a draw!!!";
	        end.innerHTML = "No Winner. Is a draw!!!";
	        document.querySelector(".endgame").style.display = "block";
	      }
	    }
    };
